...
This repository contains my NixOS configuration. The README serves as a step-by-step guide to the initial configuration and to understanding NixOS.
...
## Using Multiple Wayland Window Managers

This guide explains how to configure and use multiple Wayland window managers (Hyprland and Niri) simultaneously in your NixOS system.

### Overview

With this configuration, you can:

- Install both Hyprland and Niri
- Switch between them at the login screen
- Maintain consistent configuration between both window managers
- Share common utilities (notification center, status bar, etc.)

### Configuration Structure

The configuration is split into system-level and user-level components:

#### System Level (`modules/system/desktop/wayland-wm.nix`)

This module:

- Installs core packages needed by Wayland compositors
- Sets up XDG portals for file choosing, screenshots, etc.
- Configures system services required by window managers
- Makes both window managers available at the system level

#### User Level (`modules/home/wm.nix`)

This module:

- Manages personal configuration for each window manager
- Sets up notification center, status bar, lock screen, etc.
- Handles keyboard layout and other user preferences
- Provides a consistent experience across window managers

### How to Enable Multiple Window Managers

#### 1. System Configuration

Add the following to your host configuration file (e.g., `hosts/pcgame/default.nix`):

```nix
# Import the Wayland WM system module
imports = [
  # ... other imports
  ../../modules/system/desktop/wayland-wm.nix
];

# Enable both window managers at the system level
wayland-wm = {
  enable = true;
  enableHyprland = true;
  enableNiri = true;
...
# Configure the display manager to show all available sessions
services.displayManager = {
  defaultSession = "gnome"; # Optional default
  gdm = {
    enable = true;
    wayland = true;
  };
...
Add the following to your user configuration file (e.g., `home/stefano/default.nix`):

```nix
# Import the Wayland WM user module
imports = [
  # ... other imports
  ../../modules/home/wm.nix
];

# Enable both window managers at the user level
wayland-wm = {
  enable = true;
  enableHyprland = true;
  enableNiri = true;
...
2. At the GDM (or other display manager) login screen, click on the gear icon ⚙️ or session selector
3. Choose between:
   - GNOME
   - Hyprland
   - Niri
...
The selected window manager will start, while maintaining your consistent configuration.

### Shared Features

Both window managers will share:

- **Notification Center**: SwayNC (`SUPER+N` to toggle)
- **Status Bar**: Waybar (customized for each WM)
- **App Launcher**: Fuzzel (`SUPER+D` to launch)
- **Screenshot Tool**: Grim + Slurp (`SUPER+SHIFT+S` or `Print` key)
- **Lock Screen**: SwayLock (`SUPER+ALT+L` to lock)
- **Power Menu**: wlogout (`SUPER+Escape` to show)

### Customization

#### Changing Configuration for a Specific Window Manager

If you want to customize settings for only one window manager:

- **Hyprland**: Edit the Hyprland section in `modules/home/wm.nix`
- **Niri**: Edit the Niri configuration text in `modules/home/wm.nix`

#### Adding More Window Managers

To add support for other Wayland compositors (like Sway or River):

1. Add options in both the system and user level modules
2. Create configuration sections for the new window manager
3. Add installation instructions to the system module

### Troubleshooting

### Both Window Managers Show in Login But Only One Works

Make sure you've enabled the window manager at both the system and user level.

#### Programs Not Working Correctly in One Window Manager

Check that all required environmental variables are set consistently.

#### Window Manager Session Not Appearing in Login Screen

Verify that the window manager is properly enabled at the system level and that your display manager is correctly configured.

### Conclusion

This setup provides the flexibility to use multiple Wayland window managers without duplicating configuration. It allows you to switch easily between them while maintaining a consistent user experience.
