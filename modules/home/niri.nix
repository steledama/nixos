...
# Home-manager configuration for Niri using common Wayland components
{
  config,
  lib,
  pkgs,
...
in {
  ...
  xdg.configFile."niri/config.kdl".source =
    lib.mkIf (config ? xdg.configFile."niri/config.kdl".source)
    config.xdg.configFile."niri/config.kdl".source;

  # Fallback configuration if no custom config is provided
  xdg.configFile."niri/config.kdl".text = lib.mkIf (!config ? xdg.configFile."niri/config.kdl".source) ''
        // Niri configuration with sane defaults
        input {
            keyboard {
                xkb {
                    layout "${keyboardLayout}"
